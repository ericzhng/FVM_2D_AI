...
class UnstructuredMesh:
    ...
    def _compute_cell_volumes(self):
        ...
        if self.dim == 2:
            ...
            for i, cell in enumerate(self.connectivity):
                ...
                self.cell_volumes[i] = (
                    np.abs(np.dot(v1 - v0, np.cross(v2 - v0, v3 - v0))) / 6.0
                )
    ...
    def _compute_normals_tangents(self):
        ...
        if self.dim == 2:
            for i, face in enumerate(self.faces):
                ...
                self.face_normals[i] = np.array(
                    [-self.face_tangents[i][1], self.face_tangents[i][0]]
                )
    ...
    def add_ghost_cells(self, boundary_conditions):
        self.boundary_types = boundary_conditions
        self.n_ghost = len(self.boundary_faces)
        ghost_centers = np.zeros((self.n_ghost, self.dim))
        ghost_connectivity = []
        ghost_map = {}
        for idx, face_idx in enumerate(self.boundary_faces):
            cell_idx = self.face_neighbors[face_idx][0]
            dr = self.cell_centers[cell_idx] - self.face_centers[face_idx]
            ghost_centers[idx] = self.face_centers[face_idx] + dr
            ghost_connectivity.append(
                self.connectivity[cell_idx]
            )  # Simplified connectivity
            ghost_map[face_idx] = self.n_cells + idx
            self.face_neighbors[face_idx].append(self.n_cells + idx)
        self.cell_centers = np.vstack([self.cell_centers, ghost_centers])
        self.connectivity = np.vstack([self.connectivity, ghost_connectivity])
        self.cell_volumes = np.append(
            self.cell_volumes, np.ones(self.n_ghost) * np.mean(self.cell_volumes)
        )
        self.n_cells += self.n_ghost
        self.cell_neighbors.extend([[] for _ in range(self.n_ghost)])
        for face_idx, ghost_idx in ghost_map.items():
            real_cell = self.face_neighbors[face_idx][0]
            self.cell_neighbors[real_cell].append((ghost_idx, face_idx))
            self.cell_neighbors[ghost_idx].append((real_cell, face_idx))
    ...
    def plot_mesh(self):
        if self.dim == 2:
            ...
            for cell_idx, cell in enumerate(self.connectivity):
                ...
                ax.text(
                    self.cell_centers[cell_idx, 0],
                    self.cell_centers[cell_idx, 1],
                    str(cell_idx),
                    color="red",
                    fontsize=10,
                    ha="center",
            ...
            for face_idx, face in enumerate(self.faces):
                ...
                ax.text(
                    self.face_centers[face_idx, 0],
                    self.face_centers[face_idx, 1],
                    str(face_idx),
                    color="blue",
                    fontsize=8,
                    ha="center",
            ...
            for cell_idx, cell in enumerate(self.connectivity):
                for edge in [(0, 1), (0, 2), (0, 3), (1, 2), (1, 3), (2, 3)]:
                    ...
                    ax.plot(
                        [p1[0], p2[0]], [p1[1], p2[1]], [p1[2], p2[2]], "b-", alpha=0.3
                    )
                ax.text(
                    self.cell_centers[cell_idx, 0],
                    self.cell_centers[cell_idx, 1],
                    self.cell_centers[cell_idx, 2],
                    str(cell_idx),
                    color="red",
                    fontsize=10,
                    ha="center",
...
if __name__ == "__main__":
    test_mesh2d_generation()
    test_mesh3d_generation()
